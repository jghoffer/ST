...
log.trace code
  if (!code) {
	...
	def strcode = code
    def user = codeID as Integer
   // code = codeint as Short
    
/*	if (code instanceof String) {
		code = code.toList().findResults {    
        if(it > ' ' && it != ',' && it != '-') code = it.toCharacter() as Short 
       }
	} else {
...
def cmds = validateAttributes()
	cmds << secure(zwave.userCodeV1.userCodeSet(userIdentifier:user, userIdStatus:1, user:code))
	if(cmds.size() > 1) {
		cmds = delayBetween(cmds, 4200)
	}
	cmds
...
def deleteCode(codeID) {
	...
	def user = codeID as Integer
    secureSequence([
			zwave.userCodeV1.userCodeSet(userIdentifier:user, userIdStatus:0),
			zwave.userCodeV1.userCodeGet(userIdentifier:user)
	], 4200)
}
